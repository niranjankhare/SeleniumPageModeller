...
//        form.appendChild( elOperation);
	...
	public static String getWelcomeForm() {
		...
		Element enterNew = new Element("input").attr("name", "oper").attr("type", "radio").attr("value", "new")
				.text("Add New Page");
		Element update = new Element("input").attr("name", "oper").attr("type", "radio").attr("value", "update")
				.attr("checked", "checked").text("Update existing Page");
	...
	public static String getPageUpdateGUIForm(String pageName) {
		...
		Element elPageHeader = new Element("font").text("Page:" + pageName);
		elPageHeader.attr("size", "+2");
		html.body().appendChild(new Element("p").appendChild(elPageHeader));
		...
		html.body().attr("onload",
				"var pageName=document.getElementById('pageName').value;loadTable('PROPSVIEW','CONTROLNAME',pageName);");
