...
public class SessionInventory {
	...
	private static LinkedHashMap<SessionId, WebDriver> sessionInventory = new LinkedHashMap<SessionId, WebDriver>();

	public static WebDriver getDriver(SessionId session) {
		...
		WebDriver driver = sessionInventory.get(session);
		if (driver == null) {
			if (gridHub == null)
	...
	private static WebDriver getDriver(Long threadId) {
		WebDriver driver = sessionInventory.get(threadId);
		if (driver == null) {
			// driver =new ChromeDriver();

			driver = new ChromeDriver();
			SessionId session = ((ChromeDriver)driver).getSessionId();
			sessionInventory.put(session, driver);
	...
	private static WebDriver setWebDriver(String browser, ChromeOptions options) {
		...
		sessionInventory.put(session, webDriver);
		return webDriver;
	...
	private static WebDriver setWebDriver(String browser, DesiredCapabilities desiredCapabilities) {
		...
		// webDriver.manage().window().maximize();
		// }
		sessionInventory.put(((RemoteWebDriver) webDriver).getSessionId(), webDriver);
		return webDriver;
	}

	public static void closeSession(SessionId session) {
		Long sessionKey = Thread.currentThread().getId();
		System.out.println("Closing Session:"+ sessionKey);
		getDriver(session).close();
		sessionInventory.remove(sessionKey);
	}
}
